	USE RAW
	GO
	IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'Carnival.Call_Interaction_Activity_history') AND type in (N'U'))
	BEGIN
		PRINT 'DROP TABLE RAW.Carnival.Call_Interaction_Activity_history'
		DROP TABLE Carnival.Call_Interaction_Activity_history
		PRINT 'CREATE TABLE RAW.Carnival.Call_Interaction_Activity_history'
	END

SET QUOTED_IDENTIFIER ON
GO

CREATE TABLE [Carnival].[Call_Interaction_Activity_history](
	Call_Interaction_Activity_history_id  [INT] IDENTITY(1,1) NOT NULL,
	Call_Interaction_Activity_sk [int] NOT NULL,
	...
	Archived_dtm [datetime] NOT NULL CONSTRAINT DF_Carnival_CIAH_Archived_dtm DEFAULT GETDATE(),
	 CONSTRAINT [PK_Carnival_CIAH_Call_Interaction_Activity_History_SK] PRIMARY KEY CLUSTERED 
(
	[Call_Interaction_Activity_history_id] ASC
